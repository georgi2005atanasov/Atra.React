...
const Info = () => {
    ...
    const handleDelete = async () => {
        try {
            ...
            navigate('/private/components/all');
        } catch (error) {
    ...
    const goToUpdateComponent = () => navigate(`/private/components/update/${component.id}`);
                            ...
                            <div className="card-body p-0">
                                <div className="table-responsive">
                                    <table className="table table-striped table-bordered mb-0">
                                        ...
                                        <tbody>
                                            {component.detailsPrices.map((detail, index) => (
                                                <tr key={index}>
                                                    <td>
                                                        {detail.priceId ? (
                                                            <Link
                                                                to={`/private/details/info/${detail.priceId}`}
                                                    ...
                                                    <td className="text-end">{detail.price} лв.</td>
                                                    <td className="text-end">{detail.count}</td>
                                                    <td>{detail.unit !== null ? PRICE_UNIT_LABELS[detail.unit] : '-'}</td>
                                ...
                                .map((detail, index) => (
                                    <div key={index} className="col-md-4 col-lg-3">
                                        <div className="card">
                                            ...
                                            />
                                            <div className="card-body">
                                                <h6 className="card-title text-center">{detail.detailName}</h6>
                                            </div>
                                        </div>
                                    </div>
                                ))}
...
export async function loader({ params }) {
    try {
        ...
        return {
            // ModifiedOn - must be added
            // ModifiedBy - must be added
            component: resDetail.data.component,
            // Need to make GetById DTO on the service
            // if not, resDetail.data is enough
        };
    } catch {
        return redirect("/");
    }
